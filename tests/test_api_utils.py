...
from cfad.residue_score import (
    normalise_scores,
    rolling_pvalue,
    threshold_by_fpr,
)
...
def test_api_detect_pandas_series():
    ...
    returns = pd.Series(rng.normal(0, 0.01, 120), index=pd.date_range('2021-01-01', periods=120))
...
def test_load_spy_sample_cached(tmp_path):
    ...
    df = pd.DataFrame({'Close': [100.0, 101.0, 102.0]}, index=pd.date_range('2020-01-01', periods=3))
    df.to_csv(data_path)
    try:
