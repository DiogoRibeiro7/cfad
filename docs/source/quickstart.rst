...
Interpretation: if ``winner`` is ``"nig"``, the empirical characteristic
function is closer to a non-analytic heavy-tailed model than to the Gaussian
baseline for that sample.
